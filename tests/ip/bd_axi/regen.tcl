# One-shot recipe to produce an interface-rich Block Design (bd_axi.bd):
# AXIS FIFO (AXI_STR_RXD/TXD) bridged to an AXI BRAM controller over a
# SmartConnect, exposed as the AXI-Lite slave S_AXI plus the AXIS_RX /
# AXIS_TX stream ports. Mirrors the firmware design in fw.tcl, but stops
# at save_bd_design — wrapper generation is VivadoBd's job, not the
# fixture's. Exercises VivadoBd's interface-flattening path end-to-end.

create_project -in_memory -force -part xczu7eg-ffvc1156-2-e

file mkdir ip
create_bd_design -dir [pwd]/ip bd_axi
current_bd_design bd_axi
