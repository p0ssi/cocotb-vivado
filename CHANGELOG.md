...
- `tests/fw.tcl` targets the top block design explicitly in
  `make_wrapper`. On Vivado 2025.1 the previous `FILE_TYPE == "Block
  Designs"` filter also matched SmartConnect's nested sub-BD, which
  `make_wrapper` rejects ("can only be generated by its parent
  sub-design"); `test_fw` now runs on 2023.1 and 2025.1.
